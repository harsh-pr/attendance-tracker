...
export default function AiTimetable() {
  ...
  const getCellClassName = (type) => {
    let base = "p-3.5 text-center transition-all duration-200 border-b border-r border-slate-200 dark:border-slate-800 min-w-[140px] align-middle ";
    if (isEditMode) base += "cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-800/40 ";
    
    // Low contrast colors matching Excel design
    if (type === "free") return base + "bg-white dark:bg-[#0b0c10] text-slate-400 dark:text-slate-600";
  ...
  const renderRowCells = (dayKey) => {
    ...
    for (let i = 0; i < TIMESLOTS.length; i++) {
      ...
      const lec = schedule[i] || { subject: "", teacher: "", room: "", type: "free", colSpan: 1 };
      ...
      cells.push(
        <td
          ...
          onClick={() => handleCellClick(dayKey, i)}
        >
          {lec.subject ? (
            <div className="space-y-1 select-none">
              {lec.room && (
                <div className="text-[9px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wide">
                  {lec.room}
                </div>
              )}
              ...
              {lec.teacher && (
                <div className="text-[10px] font-medium text-slate-500 dark:text-slate-450 truncate" title={getTeacherFullName(lec.teacher)}>
    ...
    <div className="max-w-7xl mx-auto px-4 pt-6 pb-24 space-y-6 animate-fade-in text-gray-900 dark:text-slate-100 font-sans">
      ...
      {activeTab === "grid" && (
        <div className="bg-white dark:bg-[#0d0e12] border border-slate-200 dark:border-slate-800 rounded-3xl shadow-sm overflow-hidden p-1">
          <div className="overflow-x-auto">
            <table className="w-full min-w-[1100px] border-collapse text-left table-fixed">
              <thead>
                <tr className="bg-slate-50 dark:bg-slate-900/50 border-b border-slate-200 dark:border-slate-800">
                  ...
                  <th className="p-4 text-xs font-bold text-slate-500 uppercase tracking-wider border-r border-slate-200 dark:border-slate-800 text-center w-[150px]">
                    9 AM - 10 AM
                  </th>
                  <th className="p-4 text-xs font-bold text-slate-500 uppercase tracking-wider border-r border-slate-200 dark:border-slate-800 text-center w-[150px]">
                    10 AM - 11 AM
                  ...
                  <th className="p-4 text-xs font-bold text-slate-500 uppercase tracking-wider text-center w-[150px]">
                    4 PM - 5 PM
                  </th>
                </tr>
              </thead>
                ...
                {DAYS.map((dayKey, dayIdx) => {
                  ...
                  if (isWeekend) {
                    return (
                      <tr
                        key={dayKey}
                        className="border-b border-slate-200 dark:border-slate-800/80 bg-slate-50/50 dark:bg-slate-900/10"
                      >
                        <td className="p-4 font-black uppercase text-[10px] tracking-widest border-r border-slate-200 dark:border-slate-800 bg-slate-50/70 dark:bg-slate-900/30 text-slate-500">
                          {dayKey.substring(0, 3)}
                        </td>
                        <td
                          colSpan={9}
                          className="p-5 text-center font-bold text-blue-600 dark:text-blue-400 uppercase tracking-[0.25em] text-xs font-[Poppins]"
                        >
                          HOLIDAY
                        </td>
                      </tr>
                  ...
                  return (
                    <tr
                      key={dayKey}
                      className="border-b border-slate-200 dark:border-slate-800/80 hover:bg-slate-50/20 dark:hover:bg-slate-900/10 transition duration-150"
                    >
                      {/* Day Name Column */}
                      <td className="p-4 font-black uppercase text-[10px] tracking-widest border-r border-slate-200 dark:border-slate-800 bg-slate-50/70 dark:bg-slate-900/30">
                        {dayKey.substring(0, 3)}
                      </td>

                      {/* Render slots 1 & 2 */}
                      {renderRowCells(dayKey).slice(0, 2)}

                      {/* Recess Column Span */}
                      {dayIdx === 0 && (
                        <td
                          rowSpan={5}
                          className="bg-slate-100/50 dark:bg-[#161821] text-blue-600 dark:text-blue-400 font-black text-center align-middle border-r border-slate-200 dark:border-slate-800 leading-normal py-4"
                        >
                          <div className="flex flex-col items-center justify-center font-[Poppins] gap-1.5 select-none text-[10px]">
                            {BREAKS.recess.letters.map((l, i) => (
                              <span key={i} className="font-black leading-none">{l}</span>
                            ))}
                          </div>
                        </td>
                      )}
                      ...
                      {/* Render slots 3 & 4 */}
                      {renderRowCells(dayKey).slice(2, 4)}
      ...
      {isMetadataModalOpen && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 max-w-md w-full rounded-3xl p-6 shadow-2xl space-y-4 animate-scale-in text-gray-900 dark:text-white">
      ...
      {editingCell && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 max-w-sm w-full rounded-3xl p-6 shadow-2xl space-y-4 animate-scale-in text-gray-900 dark:text-white">
            ...
            <div className="space-y-3">
              ...
              <label className="block text-xs font-bold text-slate-550 uppercase tracking-wider">
                ...
                <select
                  ...
                  className="w-full mt-1.5 px-3 py-2 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-xs font-bold focus:outline-none text-gray-900 dark:text-white"
                >
            ...
            <div className="pt-2 flex justify-end gap-2">
              <button
                ...
                className="px-4 py-2 bg-slate-200 hover:bg-slate-300 dark:bg-slate-700 dark:hover:bg-slate-650 text-slate-800 dark:text-slate-200 text-xs font-bold rounded-xl cursor-pointer"
              >
                Cancel
              </button>
              <button
